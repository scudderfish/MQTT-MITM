...
Will Message: {format_multi_line(" ",self.payload.get('WillMessage', b'None'))}
User Name: {self.payload.get('UserName')}
Password: {format_multi_line(" ",self.payload.get('Password', b'None'))}
"""
        elif self.packet_type == self.SUBSCRIBE:
    ...
    def _parse_length_prefixed_bytes(self, offset):
        ...
        field_content_bytes = self._packet[offset +
                                           2: offset + 2 + field_length]
    ...
    def _parse_publish_variable_headers(self):
        ...
        field_length, field_content_bytes = self._parse_length_prefixed_bytes(
            offset)
    ...
    def _parse_subscribe_payload(self):
        ...
        while len(self._packet) - offset > 0:
            field_length, topic_filter_bytes = self._parse_length_prefixed_bytes(
                offset)
            offset += field_length
    ...
    def _parse_connect_payload(self):
        ...
        while len(self._packet) - offset > 0:
            field_length, field_content = self._parse_length_prefixed_bytes(
                offset)
