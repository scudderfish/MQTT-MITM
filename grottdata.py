...
class GrottPvOutLimit:
    ...
    def ok_send(self, pvserial: str, conf) -> bool:
        ...
        if self.register.get(pvserial):
            ok = True if self.register.get(
                pvserial) + conf.pvuplimit * 60 < now else False
            if ok:
...
def procdata(conf, data):
    ...
    if conf.compat is False:
        ...
        if (conf.invtype == "default"):
            ...
            if (ndata > 50) and not is_smart_meter:
                ...
                inverterSerial = None
                try:
                    inverterSerial = codecs.decode(
                        result_string[76:96], "hex").decode('ASCII')
                    if conf.verbose:
                ...
                if inverterSerial:
                    ...
                    try:
                        ...
                        print(
                            "\t - Matched inverter serial to inverter type", inverterType)
                    except:
                        inverterType = "default"
                        print(
                            "\t - Inverter serial not recognised - using inverter type", inverterType)
        ...
        try:
            ...
            logdict = {}
            logdict = bytes.fromhex(result_string[conf.recorddict[layout]["logstart"]["value"]:len(
                result_string)-4]).decode("ASCII").split(",")
        except:
            pass
        ...
        for keyword in conf.recorddict[layout].keys():
            ...
            if keyword not in ("decrypt", "date", "logstart", "device"):
                ...
                try:
                    if ((include) or (conf.includeall)):
                        ...
                        if keytype == "text":
                            ...
                            definedkey[keyword] = codecs.decode(
                                definedkey[keyword], "hex").decode('utf-8')
                            # print(definedkey[keyword])
                        if keytype == "num":
                            # else:
                            definedkey[keyword] = int(result_string[conf.recorddict[layout][keyword]["value"]
                                                      :conf.recorddict[layout][keyword]["value"]+(conf.recorddict[layout][keyword]["length"]*2)], 16)
                        if keytype == "numx":
                        ...
                        if keytype == "log":
                            # Proces log fields
                            definedkey[keyword] = logdict[conf.recorddict[layout]
                                                          [keyword]["pos"]-1]
                        if keytype == "logpos":
                    ...
                    if conf.verbose:
                        print("\t - grottdata - error in keyword processing : ",
        ...
        try:
            ...
            try:
                ...
                conf.recorddict[layout]["pvserial"] = {
                    "value": 0, "type": "text"}
                if conf.verbose:
                    print(
                        "\t - pvserial not found and device not specified used configuration defined invertid:", definedkey["pvserial"])
        ...
        if dateoffset > 0 and (conf.gtime != "server" or buffered == "yes"):
            ...
            pvdate = pvyear + "-" + pvmonth + "-" + pvday + \
                "T" + pvhour + ":" + pvminute + ":" + pvsecond
            ...
            try:
                ...
                # Indicate of date/time is from server (used for buffered data)
        ...
        if serialfound == True:
            ...
            if conf.verbose:
                print("\t - " + 'Growatt processing values for: ',
            ...
            definedkey["pvstatus"] = int(
                result_string[snstart+conf.offset*2+15*2:snstart+conf.offset*2+15*2+4], 16)
            ...
            if definedkey["pvstatus"] == 0 or definedkey["pvstatus"] == 1:
                definedkey["pvpowerin"] = int(
                    result_string[snstart+conf.offset*2+17*2:snstart+conf.offset*2+17*2+8], 16)
                definedkey["pv1voltage"] = int(
                    result_string[snstart+conf.offset*2+21*2:snstart+conf.offset*2+21*2+4], 16)
                definedkey["pv1current"] = int(
                    result_string[snstart+conf.offset*2+23*2:snstart+conf.offset*2+23*2+4], 16)
                definedkey["pv1watt"] = int(
                    result_string[snstart+conf.offset*2+25*2:snstart+conf.offset*2+25*2+8], 16)
                definedkey["pv2voltage"] = int(
                    result_string[snstart+conf.offset*2+29*2:snstart+conf.offset*2+29*2+4], 16)
                definedkey["pv2current"] = int(
                    result_string[snstart+conf.offset*2+31*2:snstart+conf.offset*2+31*2+4], 16)
                definedkey["pv2watt"] = int(
                    result_string[snstart+conf.offset*2+33*2:snstart+conf.offset*2+33*2+8], 16)
                definedkey["pvpowerout"] = int(
                    result_string[snstart+conf.offset*2+37*2:snstart+conf.offset*2+37*2+8], 16)
                definedkey["pvfrequentie"] = int(
                    result_string[snstart+conf.offset*2+41*2:snstart+conf.offset*2+41*2+4], 16)
                definedkey["pvgridvoltage"] = int(
                    result_string[snstart+conf.offset*2+43*2:snstart+conf.offset*2+43*2+4], 16)
                definedkey["pvenergytoday"] = int(
                    result_string[snstart+conf.offset*2+67*2:snstart+conf.offset*2+67*2+8], 16)
                definedkey["pvenergytotal"] = int(
                    result_string[snstart+conf.offset*2+71*2:snstart+conf.offset*2+71*2+8], 16)
                definedkey["pvtemperature"] = int(
                    result_string[snstart+conf.offset*2+79*2:snstart+conf.offset*2+79*2+4], 16)
                definedkey["pvipmtemperature"] = int(
                    result_string[snstart+conf.offset*2+97*2:snstart+conf.offset*2+97*2+4], 16)
                dataprocessed = True

            else:
                if conf.verbose:
                    print("\t - " + 'No valid monitor data, PV status: :',
                          definedkey["pvstatus"])
    ...
    if dataprocessed:
        ...
        if conf.verbose:
            if conf.compat:
                ...
                definedkey["pvserial"] = codecs.decode(
                    definedkey["pvserial"], "hex").decode('utf-8')
                print("\t - " + "Grott values retrieved:")
                print("\t\t - " + "pvserial:         ", definedkey["pvserial"])
                print("\t\t - " + "pvstatus:         ", definedkey["pvstatus"])
                print("\t\t - " + "pvpowerin:        ",
                      definedkey["pvpowerin"]/10)
                print("\t\t - " + "pvpowerout:       ",
                      definedkey["pvpowerout"]/10)
                print("\t\t - " + "pvenergytoday:    ",
                      definedkey["pvenergytoday"]/10)
                print("\t\t - " + "pvenergytotal:    ",
                      definedkey["pvenergytotal"]/10)
                print("\t\t - " + "pv1watt:          ",
                      definedkey["pv1watt"]/10)
                print("\t\t - " + "pv2watt:          ",
                      definedkey["pv2watt"]/10)
                print("\t\t - " + "pvfrequentie:     ",
                      definedkey["pvfrequentie"]/100)
                print("\t\t - " + "pvgridvoltage:    ",
                      definedkey["pvgridvoltage"]/10)
                print("\t\t - " + "pv1voltage:       ",
                      definedkey["pv1voltage"]/10)
                print("\t\t - " + "pv1current:       ",
                      definedkey["pv1current"]/10)
                print("\t\t - " + "pv2voltage:       ",
                      definedkey["pv2voltage"]/10)
                print("\t\t - " + "pv2current:       ",
                      definedkey["pv2current"]/10)
                print("\t\t - " + "pvtemperature:    ",
                      definedkey["pvtemperature"]/10)
                print("\t\t - " + "pvipmtemperature: ",
        ...
        if conf.nomqtt != True:
            ...
            try:
                ...
                publish.single(mqtttopic, payload=jsonmsg, qos=0, retain=conf.mqttretain, hostname=conf.mqttip,
                               port=conf.mqttport, client_id=conf.inverterid, keepalive=60, auth=conf.pubauth)
                if conf.verbose:
        ...
        if conf.pvoutput:
            ...
            if not pvidfound:
                if conf.verbose:
                    print("\t - " + "pvsystemid not found for inverter : ",
                          definedkey["pvserial"])
                return
            if not pvout_limit.ok_send(definedkey["pvserial"], conf):
                ...
                return
            if conf.verbose:
                print("\t - " + "Grott send data to PVOutput systemid: ",
                      pvssid, "for inverter: ", definedkey["pvserial"])
            pvheader = {
            ...
            if header[14:16] != "20":
                ...
                reqret = requests.post(
                    conf.pvurl, data=pvdata, headers=pvheader)
                if conf.verbose:
                ...
                if conf.verbose:
                    print("\t\t - ", pvdata2)
                reqret = requests.post(
                    conf.pvurl, data=pvdata1, headers=pvheader)
                if conf.verbose:
                    print("\t - " + "Grott PVOutput response SM1: ")
                if conf.verbose:
                    print("\t\t - ", reqret.text)
                reqret = requests.post(
                    conf.pvurl, data=pvdata2, headers=pvheader)
                if conf.verbose:
    ...
    if conf.influx:
        ...
        try:
            ...
            if conf.verbose:
                print(
                    "\t - " + "Grott PYTZ Library not installed in Python, influx processing disabled")
            conf.inlyx = False
            return
        try:
            ...
            if conf.verbose:
                if conf.tmzone == "local":
                    ...
                    print("\t - " + "Grott unknown timezone : ",
                          conf.tmzone, ", default timezone used")
            conf.tmzone = "local"
        ...
        if conf.tmzone == "local":
            ...
            utc_dt = datetime.strptime(
                jsondate, "%Y-%m-%dT%H:%M:%S") + timedelta(seconds=curtz)
        else:
        ...
        if conf.verbose:
            print("\t - " + "Grott original time : ", jsondate,
                  "adjusted UTC time for influx : ", ifdt)
        ...
        try:
            if (conf.influx2):
                ...
                ifresult = conf.ifwrite_api.write(
                    conf.ifbucket, conf.iforg, ifjson)
                # print(ifresult)
            else:
                if conf.verbose:
